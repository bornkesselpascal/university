...
{
    int sem_key;
    if ((sem_key = ftok("/home/bornk/Dokumente/Developer/university/c/vs/ue2", '2')) < 0)
    {
        perror("Fehler bei ftok");
        exit(1);
    }

    if ((sem_id = semget(sem_key, (philosophers+1), IPC_CREAT|0666)) < 0)
    {
...
void essen(size_t process, int gabel_1, int gabel_2)
{
    ...
    P(philosophers);
    printf("ER%ld (G1%d G2%d) - Philosoph %ld hat den Raum betreten.\n", process, gabel_1, gabel_2, process);
    ...
    printf("EV%ld (G1%d G2%d) - Philosoph %ld hat den Raum verlassen.\n", process, gabel_1, gabel_2, process);
    V(philosophers);
}
